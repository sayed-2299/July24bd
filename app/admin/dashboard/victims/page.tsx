...
export default function AdminVictimsPage() {
  ...
  const fetchVictims = async () => {
    try {
      ...
      // Map the tab status to the actual verification status for admin
      let status = activeTab
      if (activeTab === "pending") {
        status = "uno-verified" // Show UNO-verified victims that need admin verification
      } else if (activeTab === "verified") {
        status = "admin-verified" // Show admin-verified victims
  ...
  const handleVerification = async (victimId: string, status: "admin-verified" | "rejected") => {
    try {
      ...
      toast({
        title: "Success",
        description: `Victim ${status === "admin-verified" ? "verified" : "rejected"} successfully`,
      })
                            ...
                            <Dialog>
                              ...
                              <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                                ...
                                <VictimPreview
                                  victim={victim}
                                  role="admin"
                                  onApprove={activeTab === "pending" ? () => handleVerification(victim._id, "admin-verified") : undefined}
                            ...
                            {activeTab === "pending" && (
                              <>
                                <Button
                                  ...
                                  onClick={() => handleVerification(victim._id, "admin-verified")}
                                >
